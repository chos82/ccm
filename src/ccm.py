...
class OCVDetector:
    ...
    def run(self):
        ...
        while True:
            ...
            _ret, threshold_img = cv.threshold(frame_gray, 100, 255, cv.THRESH_BINARY)
            contours, _hierarchy  = cv.findContours(threshold_img, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
            _img = cv.drawContours(frame, contours, -1, (0,255,75), 2)

            # detect and draw bounding box
#            draw = []
#            detected_objects = self.detect_bounding_box(res_frame)
